class SearchController < ApplicationController
  ...
  def search_item
    ...
    if params[:search_input].empty? || params[:search_input].to_s.eql?(' ')
      ...
      @item_search_word = get_search_words(params[:taggable_type].to_s)
      @the_instances = Item.find_all_by_item_type(params[:taggable_type].to_s)
    end
  ...
  def get_search_words(type)
    #return Tag.find_all_by_parent_id_and_taggable_type_and_searchable(nil, the_class(type).to_s, '1')
    return Tag.find_all_by_parent_id_and_taggable_type_and_searchable(nil, the_class(type, false).to_s, true)
  end
  
  def the_class(type, superclass)
...
#       searchable = Tag.find(tag_value.tag_id)
#       unless searchable.nil?
#         searchable = searchable.searchable.to_s
...
#       searchable = Tag.find(tag_name.tag_id)
#       debugger
#       unless searchable.nil?
#         searchable = searchable.searchable.to_s
